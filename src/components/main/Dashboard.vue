  ...
  <div class="org-dashboard font-sr">
              ...
              <div class="holders-toolbar">
                ...
                <div class="holders-actions">
                  ...
                  <button
                    ...
                    :class="{
                      'holders-action-button--active': isHolderDeleteMode,
                    }"
                    @click="toggleHolderDeleteMode"
                  >
                    ជ្រើសលុប
                  </button>
                  <button
                    v-if="isHolderDeleteMode"
                    type="button"
                    class="holders-action-button holders-action-button--danger font-sr"
                    :disabled="!selectedHolderEmailsForDelete.length"
                    @click="deleteSelectedHolders"
                  >
                    លុប ({{ selectedHolderEmailsForDelete.length }})
                  </button>
                </div>
              </div>
              ...
              <div class="holders-list holders-list--inline">
                <article
                  ...
                  :class="{
                    ...
                    'holder-card--delete-selected':
                      selectedHolderEmailsForDelete.includes(holder.email),
                  }"
                  @click="handleHolderCardClick(holder)"
                >
    ...
    >
      <div class="overlay-card" @click.stop>
        ...
        <label
          v-if="
            nodeModalAction !== 'delete' &&
            nodeModalAction !== 'delete-position'
          "
          class="overlay-field"
        >
...
const modalDescription = computed(() => {
  ...
  if (nodeModalAction.value === "delete-position") {
    return "តើអ្នកចង់លុបតួនាទីនេះមែនទេ?";
  }
...
function handlePositionSelect(position) {
  ...
  isHolderDeleteMode.value = false;
  selectedHolderEmailsForDelete.value = [];
}
...
function handleHolderCardClick(holder) {
  if (!isHolderDeleteMode.value) {
    handleHolderSelect(holder);
    return;
  }

  selectedHolderEmailsForDelete.value =
    selectedHolderEmailsForDelete.value.includes(holder.email)
      ? selectedHolderEmailsForDelete.value.filter(
          (email) => email !== holder.email,
        )
      : [...selectedHolderEmailsForDelete.value, holder.email];
}
...
function toggleHolderDeleteMode() {
  isHolderDeleteMode.value = !isHolderDeleteMode.value;
  if (!isHolderDeleteMode.value) {
    selectedHolderEmailsForDelete.value = [];
  }
}

function deleteSelectedHolders() {
  if (!selectedPosition.value || !selectedHolderEmailsForDelete.value.length) {
    return;
  }

  selectedPosition.value.people = (selectedPosition.value.people || []).filter(
    (holder) => !selectedHolderEmailsForDelete.value.includes(holder.email),
  );

  const remainingHolder = selectedPosition.value.people?.[0] || null;
  selectedHolderEmail.value = remainingHolder?.email || "";
  selectedHolderEmailsForDelete.value = [];
  isHolderDeleteMode.value = false;
}
  ...
  const walk = (items) => {
    for (const item of items) {
      for (const position of item.positions || []) {
        for (const holder of position.people || []) {
          for (const systemKey of Object.keys(holder.systemAccess || {})) {
            const access = holder.systemAccess[systemKey];
            access.roleIds = (access.roleIds || []).filter(
              (id) => !removedRoleIds.includes(id),
            );
          }
        }
      }
...
function deleteRolesForCurrentSystem(roleIds) {
  ...
  system.roles = (system.roles || []).filter(
...
function handleNodeAction({ action, node, position }) {
...
function submitNodeAction() {
  if (nodeModalAction.value === "add-holder" && nodeModalName.value) {
    ...
    selectedPosition.value.people = [
      ...(selectedPosition.value.people || []),
      newHolder,
    ];
    selectedHolderEmail.value = newHolder.email;
    closeNodeModal();
    return;
  }
  ...
  if (nodeModalAction.value === "delete-position") {
    target.positions = (target.positions || []).filter(
      (position) => position.id !== nodeModalTargetPositionId.value,
    );
    ensureDashboardSelection();
  }
...
.holders-action-button--add {
  color: #1e4db7;
  background: #eaf1ff;
  border-color: #c9d9fb;
}

.holders-action-button--active {
  color: #173156;
  background: #dfe9f8;
  border-color: #b9cbe8;
}

.holders-action-button--danger {
  color: #a93226;
  background: #fdeeee;
  border-color: #f4c9c9;
}
